...
type MixtureDataSet interface {
  ...
  EvaluateLogPdf(edist []MatrixDistribution, pool ThreadPool) error
}
...
func (obj *MixtureStdDataSet) EvaluateLogPdf(edist []MatrixDistribution, pool ThreadPool) error {
  x    := obj.values
  p    := obj.p
  m, n := obj.p.Dims()
  if len(edist) != m {
  ...
  // for each thread
  d := make([][]MatrixDistribution, pool.NumberOfThreads())
  s := make([]float64, pool.NumberOfThreads())
  for threadIdx := 0; threadIdx < pool.NumberOfThreads(); threadIdx++ {
    d[threadIdx] = make([]MatrixDistribution, m)
    for j := 0; j < m; j++ {
      d[threadIdx][j] = edist[j].CloneMatrixDistribution()
    }
  }
  g := pool.NewJobGroup()
