...
type VectorId struct {
  Distributions []VectorDistribution
  t               Scalar
}

/* -------------------------------------------------------------------------- */

func NewVectorId(distributions ...VectorDistribution) (*VectorId, error) {
  if len(distributions) == 0 {
    return &VectorId{}, nil
  }
  d := make([]VectorDistribution, len(distributions))
  t := NewScalar(distributions[0].ScalarType(), 0.0)
  for i := 0; i < len(distributions); i++ {
    d[i] = distributions[i].CloneVectorDistribution()
  }
...
func (obj *VectorId) CloneMatrixDistribution() MatrixDistribution {
  return obj.Clone()
}
...
func (obj *VectorId) LogPdf(r Scalar, x Matrix) error {
  n1, m1 := obj.Dims()
  n2, m2 :=   x.Dims()
  if n1 != n2 || m1 != m2 {
  ...
  for i := 0; i < len(obj.Distributions); i++ {
    if err := obj.Distributions[i].LogPdf(obj.t, x.Row(i)); err != nil {
      return err
    }
    r.Add(r, obj.t)
  }
...
func (obj *VectorId) ImportConfig(config ConfigDistribution, t ScalarType) error {
  ...
  for i := 0; i < len(config.Distributions); i++ {
    if obj, err := ImportVectorDistributionConfig(config.Distributions[i], t); err != nil {
      return err
    } else {
...
func (obj *VectorId) ExportConfig() (config ConfigDistribution) {
  ...
  return NewConfigDistribution("vector id connector", nil, distributions...)
}
