...
type Mixture struct {
  generic.Mixture
  Edist []MatrixDistribution
}

/* -------------------------------------------------------------------------- */

func NewMixture(weights Vector, edist []MatrixDistribution) (*Mixture, error) {
  if mixture, err := generic.NewMixture(weights); err != nil {
    return nil, err
  } else {
    if len(edist) == 0 {
      edist = make([]MatrixDistribution, mixture.NComponents())
    } else {
...
func (obj *Mixture) Clone() *Mixture {
  edist := make([]MatrixDistribution, len(obj.Edist))
  for i := 0; i < len(obj.Edist); i++ {
    edist[i] = obj.Edist[i].CloneMatrixDistribution()
  }
  return &Mixture{*obj.Mixture.Clone(), edist}
}

func (obj *Mixture) CloneMatrixDistribution() MatrixDistribution {
  return obj.Clone()
}
...
func (obj *Mixture) LogPdf(r Scalar, x Matrix) error {
...
func (obj *Mixture) ImportConfig(config ConfigDistribution, t ScalarType) error {
  ...
  distributions := make([]MatrixDistribution, len(config.Distributions))
  for i := 0; i < len(config.Distributions); i++ {
    if tmp, err := ImportMatrixDistributionConfig(config.Distributions[i], t); err != nil {
      return err
    } else {
      distributions[i] = tmp
    }
