...
func NewHmm(pi Vector, tr Matrix, stateMap []int, edist []VectorDistribution) (*Hmm, error) {
  ...
  if hmm, err := generic.NewHmm(p, t, stateMap); err != nil {
    ...
    if len(edist) == 0 {
      edist = make([]VectorDistribution, hmm.NEDists())
    } else {
...
func (obj *Hmm) Clone() *Hmm {
  edist := make([]VectorDistribution, len(obj.Edist))
  for i := 0; i < len(obj.Edist); i++ {
    edist[i] = obj.Edist[i].CloneVectorDistribution()
  }
  return &Hmm{*obj.Hmm.Clone(), edist}
}

func (obj *Hmm) CloneMatrixDistribution() MatrixDistribution {
  return obj.Clone()
}
...
func (obj *Hmm) LogPdf(r Scalar, x Matrix) error {
  return obj.Hmm.LogPdf(r, HmmDataRecord{obj.Edist, x})
}

func (obj *Hmm) Posterior(r Scalar, x Matrix, states [][]int) error {
  return obj.Hmm.Posterior(r, HmmDataRecord{obj.Edist, x}, states)
}

func (obj *Hmm) PosteriorMarginals(x Matrix) ([]Vector, error) {
  return obj.Hmm.PosteriorMarginals(HmmDataRecord{obj.Edist, x})
}

func (obj *Hmm) Viterbi(x Matrix) ([]int, error) {
...
func (obj *Hmm) ImportConfig(config ConfigDistribution, t ScalarType) error {
  ...
  distributions := make([]VectorDistribution, len(config.Distributions))
  for i := 0; i < len(config.Distributions); i++ {
    if tmp, err := ImportVectorDistributionConfig(config.Distributions[i], t); err != nil {
      return err
    } else {
      distributions[i] = tmp
    }
