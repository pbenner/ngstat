...
func NewShapeHmm(pi Vector, tr Matrix, stateMap []int, edist []MatrixDistribution) (*ShapeHmm, error) {
  ...
  if hmm, err := generic.NewHmm(p, t, stateMap); err != nil {
    ...
    if len(edist) == 0 {
      edist = make([]MatrixDistribution, hmm.NEDists())
    } else {
...
func (obj *ShapeHmm) CloneMatrixDistribution() MatrixDistribution {
  return obj.Clone()
}
