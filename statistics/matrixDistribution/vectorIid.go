...
func NewVectorIid(distribution VectorDistribution, n int) (*VectorIid, error) {
  m := distribution.Dim()
...
func (obj *VectorIid) Clone() *VectorIid {
  return &VectorIid{obj.Distribution.CloneVectorDistribution(), obj.n, obj.t.CloneScalar()}
}

func (obj *VectorIid) CloneMatrixDistribution() MatrixDistribution {
  return obj.Clone()
}
...
func (obj *VectorIid) LogPdf(r Scalar, x Matrix) error {
  n1, m1 := obj.Dims()
  n2, m2 :=   x.Dims()
  t := obj.t
  if n1 != n2 || m1 != m2 {
  ...
  r.Reset()
  for i := 0; i < n1; i++ {
    if err := obj.Distribution.LogPdf(t, x.Row(i)); err != nil {
      return err
    }
    r.Add(r, t)
  }
...
func (obj *VectorIid) ExportConfig() (config ConfigDistribution) {
  ...
  return NewConfigDistribution("scalar iid connector", parameters, obj.Distribution.ExportConfig())
}
