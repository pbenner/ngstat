...
func TestHhmm1(t *testing.T) {
  ...
  c4, _ := scalarDistribution.NewCategoricalDistribution(
    NewVector(RealType, []float64{0.7, 0.3}))
  edist := []ScalarDistribution{c1, c2, c3, c4}
...
func TestHhmm2(t *testing.T) {
  ...
  edist := []ScalarDistribution{c1, c2, c3, c4, c5, c6}
...
func TestHhmm3(t *testing.T) {
  ...
  if hhmm2, err := ImportVectorDistribution(filename, BareRealType); err != nil {
    t.Error(err); return
  } else {
    t1 := hhmm1        .Tr; t1.Map(func(x Scalar) { x.Exp(x) })
    t2 := hhmm2.(*Hhmm).Tr; t2.Map(func(x Scalar) { x.Exp(x) })
