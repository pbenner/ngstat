...
type ScalarId struct {
  Distributions []ScalarDistribution
  t               Scalar
}

/* -------------------------------------------------------------------------- */

func NewScalarId(distributions ...ScalarDistribution) (*ScalarId, error) {
  if len(distributions) == 0 {
    return &ScalarId{}, nil
  }
  d := make([]ScalarDistribution, len(distributions))
  t := NewScalar(distributions[0].ScalarType(), 0.0)
  for i := 0; i < len(distributions); i++ {
    d[i] = distributions[i].CloneScalarDistribution()
  }
...
func (obj *ScalarId) CloneVectorDistribution() VectorDistribution {
  return obj.Clone()
}
...
func (obj *ScalarId) LogPdf(r Scalar, x Vector) error {
  if x.Dim() != obj.Dim() {
  ...
  for i := 0; i < len(obj.Distributions); i++ {
    if err := obj.Distributions[i].LogPdf(obj.t, x.At(i)); err != nil {
      return err
    }
    r.Add(r, obj.t)
  }
...
func (obj *ScalarId) ImportConfig(config ConfigDistribution, t ScalarType) error {
  ...
  for i := 0; i < len(config.Distributions); i++ {
    if obj, err := ImportScalarDistributionConfig(config.Distributions[i], t); err != nil {
      return err
    } else {
