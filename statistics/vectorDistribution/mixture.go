...
func NewMixture(weights Vector, edist []VectorDistribution) (*Mixture, error) {
  if mixture, err := generic.NewMixture(weights); err != nil {
    return nil, err
  } else {
    if len(edist) == 0 {
      edist = make([]VectorDistribution, mixture.NComponents())
    } else {
...
func (obj *Mixture) CloneVectorDistribution() VectorDistribution {
  return obj.Clone()
}
