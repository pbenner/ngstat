...
func Test1(t *testing.T) {
  ...
  mixture1, err := NewMixture(weights, []VectorDistribution{normal, normal}); if err != nil {
    t.Error(err); return
  }
  if err := ExportDistribution("mixture_test.json", mixture1); err != nil {
    t.Error(err)
  }
  mixture2, err := ImportVectorDistribution("mixture_test.json", BareRealType); if err != nil {
    t.Error(err); return
  }
