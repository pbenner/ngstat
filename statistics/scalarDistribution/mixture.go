...
func NewMixture(weights Vector, edist []ScalarDistribution) (*Mixture, error) {
  if mixture, err := generic.NewMixture(weights); err != nil {
    return nil, err
  } else {
    if len(edist) == 0 {
      edist = make([]ScalarDistribution, mixture.NComponents())
    } else {
...
func (obj *Mixture) CloneScalarDistribution() ScalarDistribution {
  return obj.Clone()
}
